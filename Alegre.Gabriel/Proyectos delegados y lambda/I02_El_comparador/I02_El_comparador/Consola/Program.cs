...
namespace Consola
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            Program.Comparar(primerTexto, segundoTexto, (x, z)=> x.Length - z.Length);

            // Punto 3
            Console.WriteLine($"{NewLine}2da Comparación - Texto con más palabras:");
            Program.Comparar(primerTexto, segundoTexto, (x, z)=> x.Split(' ').Length - z.Split(' ').Length);
            
            // Punto 4
            Console.WriteLine($"{NewLine}3era Comparación - Texto con más vocales:");
            Program.Comparar(primerTexto, segundoTexto, (x, z)=> ContarVocales(x)-ContarVocales(z));

            // Punto 5
            Console.WriteLine($"{NewLine}4ta Comparación - Texto con más signos de puntuación:");
            Program.Comparar(primerTexto, segundoTexto, (x, z)=> ContarSignosPuntuacion(x)-ContarSignosPuntuacion(z));
        }
        ...
        public static void Comparar(string primerTexto, string segundoTexto, DelegadoComparacion metodoComparativo)
        {
            int valorRetorno = metodoComparativo(primerTexto, segundoTexto);

            if(valorRetorno > 0)
            {
                Console.WriteLine("Primer texto -mayor- que segundo texto");
            }
            else if(valorRetorno<0)
            {
                Console.WriteLine("Primer texto -menor- que segundo texto");

            }
            else
            {
                Console.WriteLine("Los dos textos son iguales");
            }
        }
    }
}
